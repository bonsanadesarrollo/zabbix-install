...
read -s -p "Ingresa la contraseña para el usuario de la base de datos de Zabbix: " ZABBIX_DB_PASSWORD
echo
...
if confirm "¿Quieres descargar e instalar Zabbix?"; then
    ...
    apt install zabbix-agent zabbix-server-mysql zabbix-frontend-php zabbix-apache-conf
else
...
if confirm "¿Quieres importar la base de datos inicial para Zabbix?"; then
    zcat /usr/share/zabbix-sql-scripts/mysql/server.sql.gz | mariadb --default-character-set=utf8mb4 -uzabbix -p"${ZABBIX_DB_PASSWORD}" zabbix
else
